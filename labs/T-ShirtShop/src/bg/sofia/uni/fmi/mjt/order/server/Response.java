...
public record Response(Status status, String additionalInfo, Collection<Order> orders) {
    ...
    public static Response create(int id) {
        return new Response(Status.CREATED, String.format("ORDER_ID=%s", id), null);
    }
    ...
    public String toString() {
        return "{" +
                "\"status\":\"" + this.status + "\"" +
                (this.additionalInfo == null ? "" : ", \"additionalInfo\":\"" + this.additionalInfo + "\"") +
                (this.orders == null ? "" : ", \"orders\":" + this.orders) +
                "}";
    }
}